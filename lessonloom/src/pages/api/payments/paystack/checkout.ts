...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const useLive = process.env.PAYSTACK_SECRET_KEY && mode === 'live';
  if (!useLive) {
    const purchase = await prisma.purchase.create({ data: { userId, resourceId, amountCents: resource.priceCents, currency: 'NGN', status: 'succeeded' } });
    await prisma.license.upsert({ where: { userId_resourceId: { userId, resourceId } }, update: {}, create: { userId, resourceId } });
    await prisma.paymentTransaction.create({ data: { userId, amountCents: resource.priceCents, currency: 'NGN', provider: 'paystack_stub', status: 'succeeded' } });
