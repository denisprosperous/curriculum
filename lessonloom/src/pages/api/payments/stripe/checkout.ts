...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const useLive = process.env.STRIPE_SECRET_KEY && mode === 'live';
  if (!useLive) {
    // Stub flow: immediately grant license
    ...
    await prisma.paymentTransaction.create({ data: { userId, amountCents: resource.priceCents, currency: 'USD', provider: 'stripe_stub', status: 'succeeded' } });
    return res.status(200).json({ ok: true, purchaseId: purchase.id, stub: true });
  }

  // Live mode placeholder: in real deployment, create Checkout Session here
  // For this build, return a client redirect URL placeholder
